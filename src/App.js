...
class App extends Component {
  ...
  handleGetRequest = async (e) => {
    ...
    const res = await axios.get(
      `https://api.nytimes.com/svc/search/v2/articlesearch.json?q=${searchTerm}&api-key=${process.env.REACT_APP_ARTICLE_CLIENT_ID}`
    );
    ...
    return (
      <Router>
        ...
        <Switch>
                ...
                <div className='container py-2'>
                  <form className='form py-2' onSubmit={this.handleGetRequest}>
                    <input
                      ...
                      name='find'
                    />
                    <button className='btn btn-block btn-dark my-1'>
                      Submit
                    </button>
          ...
          />
          <Route exact path='/Doc' render={(props) => <Doc {...props} />} />
        </Switch>
      </Router>
    );
  }
